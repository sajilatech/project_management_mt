...
$(document).ready(function() {
	...
	$('body').on('change', 'select[name=bc_bv_id]', function(){
		$.ajax({
			type: 'POST',
			url: SITEURL + 'consultation_types/visa_countries',
			...
			success: function(response) {
				$('select#bc-countries').val('').html(response);
...
function change_priority(target = '', value = '') {
	if ( $.trim(target) && $.isNumeric(value) ) {
		$.post(SITEURL + 'consultation_types/priority', { target : $.trim(target), value : $.trim(value) }, function(result) {});
	}
}
...
function change_status(target, value) {
	if ( $.trim(target) && $.trim(value) ) {
		$.ajax({
			type: 'POST',
			url: SITEURL + 'consultation_types/status',
...
function popup_add(target = '') {
	...
	if ( $.trim(target) ) {
		$.ajax({
			type: 'GET',
			url: SITEURL + 'consultation_types/add',
			...
			success: function(response) {
				if ( response.status == 200 ) {
					...
					$('.iconuploader').imageUploader({
						...
						maxSize: 1048576,
						label: 'Choose an image size below 1 MB. Recommended size is 200 KB.'
					}).find('input[type=file]').attr('name', 'icon').removeAttr('multiple');
					$('.selectpicker').selectpicker('refresh');
				} else {
					$.toast({
						hideAfter: 5000,
...
function popup_edit(target = '') {
	...
	if ( $.trim(target) ) {
		$.ajax({
			type: 'GET',
			url: SITEURL + 'consultation_types/edit',
			data: { target : $.trim(target) },
			dataType: 'json',
			beforeSend: function() {
				$popup = '';
			...
			success: function(response) {
				if ( response.status == 200 ) {
					...
					$('.iconuploader').imageUploader({
						preloaded: response.icons,
						imagesInputName: 'icon',
						preloadedInputName: 'icon',
						maxFiles: 1,
						maxSize: 1048576,
						label: 'Choose an image size below 1 MB. Recommended size is 200 KB.'
					}).find('input[type=file]').attr('name', 'icon').removeAttr('multiple');
