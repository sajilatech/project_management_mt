...
function filter_appointments_staff() {
	$.ajax({
		'url': $('form#filter-form').attr('action'),
		'method': 'POST',
		'data': $('form#filter-form').serialize(),
			...
			buttons: {
				formSubmit: {
					...
					action: function () {
						...
						$.ajax({
							type: 'POST',
							url: SITEURL + 'visa_appointments/status',
			...
			buttons: {
				formSubmit: {
					...
					action: function () {
						...
						$.ajax({
							type: 'POST',
							url: SITEURL + 'visa_appointments/assign',
							data: { target : $.trim(target), value : $.trim(value), message : $.trim($message) },
							dataType: 'json',
							beforeSend: function() {
								$.blockUI({ message : '' });
			...
			buttons: {
				formSubmit: {
					...
					action: function () {
						...
						$.ajax({
							type: 'POST',
							url: SITEURL + 'visa_appointments/spamer',
							data: { target : $.trim(target), value : $.trim(value), message : $.trim($message) },
							dataType: 'json',
							beforeSend: function() {
								$.blockUI({ message : '' });
...
function trigger_popup(target = '', popup = '') {
	if ( $.trim(target) && $.trim(popup) ) {
		$.ajax({
			type: 'POST',
			url: SITEURL + 'visa_appointments/popup',
