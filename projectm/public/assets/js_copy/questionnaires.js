...
$(document).ready(function() {
	...
	$(".selectpicker-visa").selectpicker();
	...
	$("body").on('change', 'select.selectfilter', function (e) { var $select = $(this);
		...
		if ( $.inArray($.trim($select.attr('name')), ['filter_visa','filter_type','filter_subtype','filter_country','filter_status']) !== -1 ) {
			$.ajax({
				type: 'POST',
				url: SITEURL + 'questionnaires/autoload',
		...
		if ( $.inArray($.trim($select.attr('name')), ['bq_visa','bq_type','bq_subtype','bq_country','bq_marital_status','bq_parent']) !== -1 ) {
			$.ajax({
				...
				beforeSend: function() { $.blockUI({ message : '' });
					...
					if ( $.trim($select.attr('name')) == 'bq_visa' && $.inArray($.trim($select.val()), ["6"]) !== -1 ) {
						$('select[name=bq_type], select[name=bq_subtype]').removeAttr('disabled').attr('disabled','disabled');
					} else {
						$('select[name=bq_type], select[name=bq_subtype]').removeAttr('disabled');
					}
				},
				success: function(response) {
					if ( $.trim($select.attr('name')) == 'bq_visa' ) {
	...
	$("body").on('change', 'select.selectpicker-visa', function (e) { $select = $(this); $form = $(this).closest('form');
		if ( $.inArray($.trim($select.attr('name')), ['bq_visa','bq_from','bq_country','bq_marital_status','bq_parent']) !== -1 ) {
			$.ajax({
				type: 'POST',
				url: SITEURL + 'questionnaires/autoload_visa',
				data: { target : $.trim($select.attr('name')), value : $.trim($select.val()), collective : $.trim(vqchecker()) },
				dataType: 'json',
				beforeSend: function() {
					$.blockUI({ message : '' });
					/* if ( $.trim($select.attr('name')) == 'bq_country' ) {
						$form.find('select[name=bq_parent], select[name=bq_option]').val('').html('');
					} else if ( $.trim($select.attr('name')) == 'bq_marital_status' ) {
						$form.find('select[name=bq_parent], select[name=bq_option]').val('').html('');
					} else if ( $.trim($select.attr('name')) == 'bq_parent' ) {
						$form.find('select[name=bq_option]').val('').html('');
					} */
				},
				success: function(response) {
					if ( $.inArray($.trim($select.attr('name')), ['bq_from','bq_country','bq_marital_status']) !== -1 ) {
						$form.find('select[name=bq_parent]').html(response.questions);
						$form.find('select[name=bq_parent] option:first').prop('selected', true);
						$form.find('select[name=bq_option]').html(response.options);
						$form.find('select[name=bq_option] option:first').prop('selected', true);
						$form.find('select.selectpicker-visa').selectpicker('refresh');
					} else if ( $.trim($select.attr('name')) == 'bq_parent' ) {
						$form.find('select[name=bq_option]').html(response.options);
						$form.find('select[name=bq_option] option:first').prop('selected', true);
						$form.find('select.selectpicker-visa').selectpicker('refresh');
	...
	$('body').on('change', 'select.disable-another', function() {
		$valuee = $(this).val();
		$target = $(this).data('target');
		if ( $('select#' + $target).length ) {
			$('select#' + $target).find('option').removeAttr('disabled');
			$('select#' + $target).val('change').trigger('change');
			if ( $.isArray($valuee) && $valuee.length ) {
				$.each($valuee, ($key, $val) => {
					if ( $('select#' + $target).find('option#option-' + $val) ) {
						$('select#' + $target).find('option#option-' + $val).attr('disabled', 'disabled');
					}
				});
			}
			
			$('select#' + $target).selectpicker('refresh');
		}
	});
});

function qchecker($result = []) {
	if ( $('select.qchecker').length ) {
		$('select.qchecker, input.qchecker').each(function(i) {
...
function filter_questionnaire() {
	$.ajax({
		'url': SITEURL + 'questionnaires/filter',
		'method': 'POST',
...
function change_status(target, value) {
	if ( $.trim(target) && $.trim(value) ) {
		$.ajax({
			type: 'POST',
			url: SITEURL + 'questionnaires/status',
...
function change_priority(thiss) {
	...
	if ( $.trim($target) && $.isNumeric($value) ) {
		$.post(SITEURL + 'questionnaires/priority', { target : $.trim($target), value : $.trim($value) }, function(result) {});
	}
}
...
function delete_row(target = '') {
	if ( $.trim(target) ) {
		if ( confirm("Are you sure you want to delete ?") ) {
			$.ajax({
				type: 'POST',
				url: SITEURL + 'questionnaires/remove',
...
function copy_questionnaire($target = '', $country = '') {
	...
	if ( $popupp.length ) {
		if ( $.trim($target) ) {
			$.post(SITEURL + 'questionnaires/autocountry', { target : $.trim($target) }).done(function(data) {
				$popupp.find('input[name=target]').val($.trim($target));
				$popupp.find('select[name=country]').html($.trim(data));
				$popupp.find('select.selectpicker').selectpicker('refresh');
				$popupp.modal('show');
			});
		}
	}
...
function copy_questionnaire_visa($target = '', $country = '') {
	$popupp = $('#popup-copy-visa');
	if ( $popupp.length ) {
		if ( $.trim($target) ) {
			$.post(SITEURL + 'questionnaires/autocountry_visa', { target : $.trim($target) }).done(function(data) {
				$popupp.find('input[name=target]').val($.trim($target));
				$popupp.find('select[name=country]').html($.trim(data));
				$popupp.find('select.selectpicker').selectpicker('refresh');
				$popupp.modal('show');
