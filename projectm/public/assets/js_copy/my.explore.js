			...
			success: function(response) {
				if ( response.status == 200 ) {
					...
					$('.logouploader').imageUploader({
						...
						maxFiles: 1
					}).find('input[type=file]').attr('name', 'logo').removeAttr('multiple');
		...
		$.ajax({
			...
			success: function(response) {
				if ( response.status == 200 ) {
					...
					$('.textareaedit').wysihtml5();
					$('.logouploader').imageUploader({
						preloaded: response.logo,
						imagesInputName: 'logo',
						preloadedInputName: 'logo',
						maxFiles: 1
					}).find('input[type=file]').attr('name', 'logo').removeAttr('multiple');
					$('.banneruploader').imageUploader({
