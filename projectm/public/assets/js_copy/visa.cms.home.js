...
function change_priority(target = '', value = '') {
	if ( $.trim(target) && $.isNumeric(value) ) {
		$.post(SITEURL + 'visa_cms_home/priority', { target : $.trim(target), value : $.trim(value) }, function(result) {});
	}
}
...
function delete_row(target = '') {
	if ( $.trim(target) ) {
		if ( confirm("Are you sure you want to delete ?") ) {
			$.ajax({
				type: 'POST',
				url: SITEURL + 'visa_cms_home/remove',
...
function change_status(target, value) {
	if ( $.trim(target) && $.trim(value) ) {
		$.ajax({
			type: 'POST',
			url: SITEURL + 'visa_cms_home/status',
...
function popup_edit(target = '') {
	...
	if ( $.trim(target) ) {
		$.ajax({
			type: 'GET',
			url: SITEURL + 'visa_cms_home/edit',
			data: { target : $.trim(target) },
			dataType: 'json',
			beforeSend: function() {
				$popup = '';
    ...
    if ( $('div.'+ $target).length ) {
		if ( $target == 'more-types' ) {
			...
			$('div.'+ $target).find('ul.nav-tabs').append(
				'<li class="active" id="typeof_'+ $row +'_link"><a href="#typeof_'+ $row +'" data-toggle="tab" aria-expanded="true">Type '+ $row +'</a></li>'
			);
			
			$('div.'+ $target).find('div.tab-pane.active').removeClass('active');
			$('div.'+ $target).find('div.tab-content').append(
				'<div class="tab-pane active" id="typeof_'+ $row +'">' +
					'<div class="form-group">' +
						'<div class="row">' +
							'<div class="col-md-7">' +
								'<label for="type-'+ $row +'-title" class="col-form-label">Title</label>' +
								'<input type="text" name="types['+ $key +'][title]" id="type-'+ $row +'-title" class="form-control" placeholder="Give a title" />' +
							'</div>' +
							'<div class="col-md-5">' +
								'<label for="type-'+ $row +'-image" class="col-form-label">Image</label>' +
								'<input type="file" name="type_image['+ $key +']" id="type-'+ $row +'-image" class="form-control" />' +
							'</div>' +
						'</div>' +
					'</div>' +
					'<div class="form-group">' +
						'<label for="type-'+ $row +'-description" class="col-form-label">Description</label>' +
						'<textarea name="types['+ $key +'][description]" id="type-'+ $row +'-description" class="form-control textarea-'+ $row +'" placeholder="Give a description"></textarea>' +
					'</div>' +
				'</div>'
			);
    ...
    if ( $('div.'+ $target).length ) {
		if ( $target == 'more-types' ) {
			...
			if ( $row > 1 ) {
				...
				$('div.'+ $target).find('div#typeof_'+ $row).remove();
				$('div.'+ $target).find('li#typeof_'+ $row + '_link').remove();
				$('div.'+ $target).find('div#typeof_'+ ($row - 1)).addClass('active');
				$('div.'+ $target).find('li#typeof_'+ ($row - 1) + '_link').addClass('active');
			}
		}
	}
}
